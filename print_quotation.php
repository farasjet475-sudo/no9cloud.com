...
if (empty($_SESSION['pos_quotation']) || !is_array($_SESSION['pos_quotation'])) {
    flash('error', 'No quotation data found.');
    redirect('pos.php');
}

$doc = $_SESSION['pos_quotation'];

if ((int)($doc['branch_id'] ?? 0) !== (int)current_branch_id()) {
...
<head>
<meta charset="utf-8">
<title>Quotation - <?php echo e($doc['doc_no'] ?? ''); ?></title>
...
.topbar{height:8px;background:linear-gradient(90deg,#7c3aed,#a855f7,#2563eb)}
.inner{padding:30px}
...
.badge{display:inline-block;padding:10px 16px;border-radius:999px;font-weight:800;font-size:13px;color:#fff;background:linear-gradient(135deg,#7c3aed,#6d28d9);text-transform:uppercase;letter-spacing:.5px}
...
<div class="sheet">
    ...
    <div class="inner">
        <div class="header">
            ...
            <div><div class="badge">Quotation</div></div>
        </div>
        ...
        <div class="footer">
            ...
            <div>Quotation validity: 7 days</div>
        </div>
    </div>
</div>

<div class="actions">
    <button class="btn btn-print" onclick="window.print()">Print Quotation</button>
